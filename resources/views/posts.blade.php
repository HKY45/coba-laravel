...
@section('container')
    ...
    @if ($posts->count())   
        <div class="card mb-3 text-center">
            ...
            @endif
            <div class="card-body">
            <h3 class="card-title"><a href="/posts?author={{ $posts[0]->author->username }}" class="text-decoration-none text-dark">{{ $posts[0]->title }}</a></h3>
            <p>
        ...
        <div class="row">
            @foreach ($posts->skip(1) as $post)
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <div class="position-absolute px-3 py-2" style="background-color: rgba(0, 0, 0, 0.815); border-radius: 5px"><a href="/posts?category={{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
                        @if ($post->image)
