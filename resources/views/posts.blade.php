...
@section('container')
    ...
    @if ($posts->count())   
        <div class="card mb-3 text-center">
            ...
            <div class="card-body">
            <h3 class="card-title"><a href="/authors/{{ $posts[0]->author->username }}" class="text-decoration-none text-dark">{{ $posts[0]->title }}</a></h3>
            <p>
        ...
        <div class="row">
            @foreach ($posts->skip(1) as $post)
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <div class="position-absolute px-3 py-2" style="background-color: rgba(0, 0, 0, 0.815); border-radius: 5px"><a href="/categories/{{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
